    ...
    */
    ...
    intptr_t HandleShortcutRemapEvent(KeyboardManagerInput::InputInterface& ii, LowlevelKeyboardEvent* data, State& state, const std::optional<std::wstring>& activatedApp) noexcept
    {
        ...
        Logger::trace(L"{} @ {}", __FUNCTIONW__, __LINE__);
            ...
            for (auto& itShortcut : state.GetSortedShortcutRemapVector(activatedApp))
            {
                ...
                Logger::trace(L"{} @ {} vkcode:{} action:{} ctrl:{} alt:{} down:{}", __FUNCTIONW__, __LINE__, data->lParam->vkCode, it->first.GetActionKey(), it->first.CheckCtrlKey(data->lParam->vkCode), it->first.CheckAltKey(data->lParam->vkCode), (data->wParam == WM_KEYDOWN || data->wParam == WM_SYSKEYDOWN));
                // If a shortcut is currently in the invoked state then skip till the shortcut that is currently invoked
                Logger::trace(L"{} @ {} {} {} {} {}", __FUNCTIONW__, __LINE__, !it->first.CheckModifiersKeyboardState(ii), data->lParam->vkCode != it->first.GetActionKey(), isShortcutInvoked, !it->second.isShortcutInvoked);
                ...
                Logger::trace(L" PREV KEY : {} @ {} {}", __FUNCTIONW__, __LINE__, state.GetPreviousModifierKey().size());
                Logger::trace(L"{} @ {} {} {} {} {}", __FUNCTIONW__, __LINE__, it->first.GetCtrlKey(it->second.modifierKeyInvoked), it->first.GetAltKey(it->second.modifierKeyInvoked), it->first.GetShiftKey(it->second.modifierKeyInvoked), it->first.GetWinKey(it->second.modifierKeyInvoked));
                ...
                Logger::trace(L"{} @ {} {}", __FUNCTIONW__, __LINE__, std::get<DWORD>(it->second.targetShortcut));
                // Check if the remap is to a key or a shortcut
                ...
                Logger::trace(L"{} @ {} {} {} {}", __FUNCTIONW__, __LINE__, !it->second.isShortcutInvoked, it->first.CheckModifiersKeyboardState(ii), it->first.IsKeyboardStateClearExceptShortcut(ii, static_cast<int>(state.GetPreviousActionKey())));
                ...
                if (!it->second.isShortcutInvoked && it->first.CheckModifiersKeyboardState(ii))
                {
                    ...
                    {
                        Logger::trace(L"{} @ {} {} {}", __FUNCTIONW__, __LINE__, isShortcutInvoked, state.GetPreviousActionKey());
                        ...
                        Logger::trace(L"{} @ {} {}", __FUNCTIONW__, __LINE__, it->first.GetActionKey());
                ...
                else if ((it->second.isShortcutInvoked || isShortcutInvoked))
                {
                    ...
                    Logger::trace(L"{} @ {} {} {} {}", __FUNCTIONW__, __LINE__, isAltRightKeyInvoked, data->lParam->vkCode == VK_LCONTROL, it->first.GetActionKey() != VK_LCONTROL);
                    ...
                    Logger::trace(L"{} @ {} {} {} ", __FUNCTIONW__, __LINE__, (it->first.CheckWinKey(data->lParam->vkCode) || it->first.CheckCtrlKey(data->lParam->vkCode) || it->first.CheckAltKey(data->lParam->vkCode) || it->first.CheckShiftKey(data->lParam->vkCode)), (data->wParam == WM_KEYUP || data->wParam == WM_SYSKEYUP));
                    Logger::trace(L"{} @ {} vkcode:{} action:{} ctrl:{} alt:{} up:{}", __FUNCTIONW__, __LINE__, data->lParam->vkCode, it->first.GetActionKey(), it->first.CheckCtrlKey(data->lParam->vkCode), it->first.CheckAltKey(data->lParam->vkCode), (data->wParam == WM_KEYUP || data->wParam == WM_SYSKEYUP));
                    ...
                    if (!remapToShortcut || (remapToShortcut && std::get<Shortcut>(it->second.targetShortcut).CheckModifiersKeyboardState(ii)))
                    {
                        ...
                        if (((data->lParam->vkCode == it->first.GetActionKey() && !it->first.HasChord()) || (data->lParam->vkCode == it->first.GetSecondKey() && it->first.HasChord()) || (state.GetPreviousActionKey() != 0 )) && (data->wParam == WM_KEYDOWN || data->wParam == WM_SYSKEYDOWN))
                        {
                            ...
                            if (state.GetPreviousActionKey() != 0 && state.GetPreviousActionKey() != data->lParam->vkCode && data->lParam->vkCode != it->first.GetActionKey())
                            {
                                Logger::trace(L"{} @ {}", __FUNCTIONW__, __LINE__);
                                size_t akey_count = 1;
                                LPINPUT akeyEventList = nullptr;

                                if (remapToShortcut)
                                {
                                    Logger::trace(L"{} @ {}", __FUNCTIONW__, __LINE__);
                                    akey_count = (dest_size) + (src_size + 1) - (2 * static_cast<size_t>(commonKeys));
                                    akeyEventList = new INPUT[akey_count]{};
                                    Helpers::SetKeyEvent(akeyEventList, 0, INPUT_KEYBOARD, static_cast<WORD>(std::get<Shortcut>(it->second.targetShortcut).GetActionKey()), KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                }
                                else if (remapToKey)
                                {
                                    Logger::trace(L"{} @ {}", __FUNCTIONW__, __LINE__);
                                    int i = 0;
                                    akey_count = dest_size + (src_size - 1) + KeyboardManagerConstants::DUMMY_KEY_EVENT_SIZE;
                                    akeyEventList = new INPUT[akey_count]{};
                                    Helpers::SetKeyEvent(akeyEventList, 0, INPUT_KEYBOARD, static_cast<WORD>(state.GetPreviousActionKey()), KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                    i++;
                                    //Helpers::SetModifierKeyEvents(it->first, it->second.modifierKeyInvoked, akeyEventList, i, true, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                }

                                UINT res = ii.SendVirtualInput(static_cast<UINT>(akey_count), akeyEventList, sizeof(INPUT));
                                delete[] akeyEventList;
                                continue;
                            }
                            ...
                            {
                                Logger::trace(L"{} @ {} {}", __FUNCTIONW__, __LINE__, static_cast<WORD>(Helpers::FilterArtificialKeys(std::get<DWORD>(it->second.targetShortcut))));
                                keyEventList = new INPUT[key_count]{};
    ...
    void SetPreviousModifierKey(const ShortcutRemapTable::iterator it, State& state)
    {
        ...
        if (it->first.GetCtrlKey(it->second.modifierKeyInvoked) != 0)
        {
            ...
            Logger::trace(L"{} @ {} {}", __FUNCTIONW__, __LINE__, state.GetPreviousModifierKey().size());
        }
